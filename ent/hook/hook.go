...
// The CommitActivityFunc type is an adapter to allow the use of ordinary
// function as CommitActivity mutator.
type CommitActivityFunc func(context.Context, *ent.CommitActivityMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CommitActivityFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CommitActivityMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CommitActivityMutation", m)
}
